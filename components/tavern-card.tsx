      ...
      >
        ...
        <div className="p-5 flex flex-col gap-3 flex-1">
          ...
          <div className="flex items-center gap-1 text-xs text-muted-foreground">
            ...
            <span className="font-semibold tracking-wide uppercase">{tavern.city}, {tavern.state}</span>
            <span className="ml-1 text-muted-foreground/50">&bull; {tavern.county} County</span>
          </div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {tavern.tags.slice(0, 3).map((tag) => (
              <span
                key={tag}
                className={`text-xs px-2 py-0.5 rounded-sm border font-medium ${tagColors[tag] ?? "bg-muted text-muted-foreground border-border"}`}
              >
                {tag}
              </span>
            ))}
